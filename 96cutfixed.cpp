...
struct frame{
	...
	if(i==header_size)cout<<"gran 1a: ";
		...
		if (m_s==2){	 if(n_offset<((gran1a+gran1b)/8)){
...
if (i==((gran1a+gran1b+gran2a+gran2b)/8)-n_offset+header_size)cout<<"* <- end of granules  ";
	...
	if(header_distance>frame_size-header_size)cout<<"Wrong header_distance"<<endl;
		...
		for(int n=0;n<frame_size;n++){
			...
			else { *(source+i_point-n_offset+n+move_frame)=data[d];
		d++;
			}
		}
...
int smpl_rate;
	...
	//check for id tags
		...
		if(fr[n].valid_frame==1){
			valid_counter++;	fr[n].valid_frame_index=valid_counter;
			}
		}
		return valid_counter;
		}

string potential_frames(char * source,long long length){
		string str;
	       for(long long i=0;i<length-4;i++){
        	if(*(source+i)==(char)0xff && isbit(source+i,8)==true &&  isbit(source+i,9)==true && isbit(source+i,10)==true){ if(isbit(source+i,11)==false && isbit(source+i,12)==true){
        	//	wrong_mpeg
        		continue;
        	}

          if(isbit(source+i,13)==false &&
          isbit(source+i,14)==false){
     //     	wrong_layer
          	continue;
          	}
    	   	...
    	   	for(int n=0;n<frames_count;n++){
    	   		if(fr[n].valid_frame==0)continue;
    	   			if(fr[n].empty_end_b<0){
    	   				for(int k=0;n+k<frames_count&&k<100;k++){	   	if(fr[n+k].valid_frame==1){
